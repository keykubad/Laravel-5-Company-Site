...
class gapi {
    ...
    public function requestReportData($report_id, $dimensions=null, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=10000) {
        ...
        if (substr($response['code'], 0, 1) == '2') {
            ...
            echo "HATA! Profil id değeri yanlış veya boş bırakılmış lütfen düzeltip tekrar deneyiniz. <a href='analytics'>Ayar Sayfasına Git</a>";
    ...
    public function fetchToken($client_email, $key_file, $delegate_email = null) {
        ...
        if (!file_exists($key_file)) {
            if ( !file_exists(__DIR__ . DIRECTORY_SEPARATOR . $key_file) ) {
                echo "HATA! Analytics p12 Develepor key yolu yada değeri yanlış veya boş bırakılmış lütfen düzeltip tekrar deneyiniz. <a href='analytics'>Ayar Sayfasına Git</a>";
        ...
        if (substr($response['code'], 0, 1) != '2' || !is_array($auth_token) || empty($auth_token['access_token'])) {
            echo "HATA! Analytics mail değeri yanlış veya boş bırakılmış lütfen düzeltip tekrar deneyiniz. <a href='analytics'>Ayar Sayfasına Git</a>";
